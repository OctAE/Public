...
$(document).ready(function(){
	...
	if (pathname == "/regentstreet/confirm-my-details") {
	          $( ".stepsHighlight" ).addClass('noAfter');
	        }
...
*/
    ...
    $('.invoiceback').after('<p id="afterCompleteMsg">Click <a href="/regentstreet/products">Open Catalogue</a> to place another order, or <a href="/regentstreet/logout">Log Out</p>');
    ...
    if (pathname.indexOf('product-edit') > -1 || pathname.indexOf('kit-edit') > -1 || pathname.indexOf('edit_rttplus') > -1) {
    	...
    	if ($('a#addToKit span:contains(kit)').length == 0) {
		        ...
		        var breadcrumbHtml = "<div style='padding: 10px 0px 30px 4px; display: block'><strong>You are at: > > <a href='/TelevisionCentreStore/products'>Television Centre Store</a>"
		       	var pathArray = decodeURIComponent(pathname).split("/");
		        var pathBefore = "";
		    		for (i = 2; i < pathArray.length-1; i++) {
		   				 pathBefore = pathBefore + "/" + pathArray[i]; 
		           		 breadcrumbHtml = breadcrumbHtml + " > <a href='/TelevisionCentreStore/products" + pathBefore + "'>" + pathArray[i] + "</a>";
					}
