...
$(document).ready(function(){
	...
	//On basket page add breadcrump steps and change "Continue Order" to "Proceed to checkout"
	 if (pathname.indexOf('basket') > -1){
	       $( "#consolenormal" ).after( '<p class="underSteps"><a href="http://mccarthyandstone.octink.com/products">CLICK HERE TO CONTINUE SHOPPING</a></p>');
	...
	if (pathname == "/confirm-my-details") {
	          $( ".stepsHighlight" ).addClass('noAfter');
	        }
...
//Change CSS when product-edit page
 ...
 if ((pathname.indexOf('product-edit2') > -1) || (pathname.indexOf('edit_rttplus') > -1)){
   		$("ul#navivert").css("border-top", "10px solid #F0F0F0");
   		$("#bannerBehind").css("height", "190px");
   		$("#bannerBehind").css("border-bottom", "50px solid #F0F0F0");
	 }




	 if ((pathname.indexOf('order-history') < 0) && (pathname.indexOf('order-confirm') < 0) && (pathname.indexOf('public') < 0) && (pathname.indexOf('home') < 0) && (pathname.indexOf('health-and-safety-signs') < 0) && (pathname.indexOf('assisted') < 0) && (pathname.indexOf('retirement') < 0)) {
   		$('td').addClass('importantWhite');
...
*/
    ...
    //$('.invoiceback').after('<p id="afterCompleteMsg">Click <a href="/products">Open Catalogue</a> to place another order, or <a href="/logout">Log Out</p>');
    ...
    if (pathname.indexOf('product-edit') > -1 || pathname.indexOf('kit-edit') > -1 ) {
    	...
    	if ($('a#addToKit span:contains(kit)').length == 0) {
		        ...
		        var breadcrumbHtml = "<div style='padding: 10px 0px 30px 4px; display: block'><strong>You are at: > > <a href='/mccarthyandstone/products'>mccarthyandstone</a>"
		       	var pathArray = decodeURIComponent(pathname).split("/");
		        var pathBefore = "";
		    		for (i = 2; i < pathArray.length-1; i++) {
		   				 pathBefore = pathBefore + "/" + pathArray[i]; 
		           		 breadcrumbHtml = breadcrumbHtml + " > <a href='/mccarthyandstone/products" + pathBefore + "'>" + pathArray[i] + "</a>";
					}
...
var GantrySignAssisted = "http://img.netprintmanager.com/red2gouk/i/695/10479/784585.8d11c804-fe31-4495-a842-a1afbcfe1c34.jpeg";

var GantrySignRetirement  = "http://img.netprintmanager.com/red2gouk/i/695/10479/784591.673f115c-b9b2-4f45-9c24-0cce016b4d08.jpeg";

switch (titleCode) {
  case "Gantry Sign - Assisted":
    iconLocation = GantrySignAssisted;
    break;  
  case "Gantry Sign - Retirement":
    iconLocation = GantrySignRetirement;
    break;  
  default:
