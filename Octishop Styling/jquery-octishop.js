...
$(document).ready(function(){
	...
	//On basket page add breadcrump steps and change "Continue Order" to "Proceed to checkout"
	 if (pathname.indexOf('basket') > -1){
	       $( "#consolenormal" ).after( '<p class="underSteps"><a href="http://red2gouk.netprintmanager.com/octishop/products">CLICK HERE TO CONTINUE SHOPPING</a></p>');
	...
	if (pathname == "/octishop/confirm-my-details") {
	          $( ".stepsHighlight" ).addClass('noAfter');
	        }
...
//Change “Edit my details” button text to “Change “delivery address”
    ...
    $( "#innerFooter" ).append( "<a href='http://www.octink.com' target='_blank' id='footer-image'><img src='https://rawgit.com/OctAE/Public/master/Octishop%20Styling/footerOctink.jpg' alt='Octink'></a>");
    ...
    if (!(pathname.indexOf('order-approval') > -1)) { 
    	$('.invoiceback p:nth-of-type(3)').html('Your files will be ready for print within 20 minutes.').after('<p>visit <a href="http://www.octink.com">octink.com</a> to find out more about your signage supplier</p>'); 
	}

    //Add navigation text below order complete message
    $('.invoiceback').after('<p id="afterCompleteMsg">Click <a href="/octishop/products">Open Catalogue</a> to place another order, or <a href="/octishop/logout">Log Out</p>');
    ...
    if (pathname.indexOf('product-edit') > -1 || pathname.indexOf('kit-edit') > -1 ) {
    	...
    	if ($('a#addToKit span:contains(kit)').length == 0) {
		        ...
		        var breadcrumbHtml = "<div style='padding: 10px 0px 30px 4px; display: block'><strong>You are at: > > <a href='/octishop/products'>octishop</a>"
		       	var pathArray = pathname.split("/");
		        var pathBefore = "";
		    		for (i = 2; i < pathArray.length-1; i++) {
		   				 pathBefore = pathBefore + "/" + pathArray[i]; 
		           		 breadcrumbHtml = breadcrumbHtml + " > <a href='/octishop/products" + pathBefore + "'>" + pathArray[i] + "</a>";
					}
